...
# process_result return the the timestamp for the 1st and last instance.
# e.g. the first and last 304731.29,Trans/s,0.511,1639350440.090
def process_result(basename, raw_results, end_time, ksink):
    # raw_results is one .out file
    first_result = True
    have_result = False
    had_results = False
    interim_result = 0.0
    ...
    for raw_result in raw_results:
        ...
        if "Interim result:" in raw_result:
            ...
            if len(fields) == 4:
                ...
                interim_start = interim_end - interim_interval
                if interim_start > end_time:
                    break
                have_result = True
            else:
                have_result = False
...
# process_result_files get the earliest instances across all .out files.
...
def get_min_max_avg_of_last_interval(prefix, start_time, end_time, intervals):
    length = int(end_time) - int(start_time)

    # (iavg, imin, imax, istart, iend)
    # first will be overwritten with peak when known

    rrdtool.create(prefix + "_intervals.rrd", '--step', '1', '--start', str(int(start_time) - 1),
    ...
    #for id, interval in enumerate(intervals[-1:], start=1):
    id = 1
    # we only look at the last interval.
    ...
    if (start >= end):
        print("for an interval, the start should be later than the end")
        raise
    # we have no interest in the size of the graph (the first two
    # items in the list) so slice just the part of interest
    result = rrdtool.graph('/dev/null', '--start', str(start), '--end', str(end),
                           'DEF:foo=%s_overall.rrd:mbps:AVERAGE' % prefix, 'VDEF:avg=foo,AVERAGE',
                           'VDEF:min=foo,MINIMUM', 'VDEF:max=foo,MAXIMUM', 'PRINT:avg:"%6.2lf"',
    ...
    return id, graph_interval, (iavg, imin, imax, start, end)
...
def units_et_al_by_prefix(prefix):
    ...
    if ("pps" in prefix) or ("tps" in prefix):
        ...
        direction = "Outbound"
    elif "search_best_num_streams" in prefix:
        direction = "Outbound"
...
def graph_overall(prefix, start_time, end_time, vrules, peak_interval_id=None, peak_average=0.0, major_interval=60,
    ...
    if peak_interval_id:
        interval_specs = ['DEF:bar=%s_intervals.rrd:avg:AVERAGE' % prefix,
                          ...
                          'LINE2:intvl#0F0F0F40:Interval average. Peak of %.3f during interval %d' % (
                          peak_average, peak_interval_id)]
...
if __name__ == '__main__':
    ...
    ksink = dict(zip(range(int(start_time), int(end_time) + 1), [0.0] * length))
    ...
    interval_id, min_graph_interval, (iavg, imin, imax, start, end) = get_min_max_avg_of_last_interval(prefix, min_timestamp, end_time, intervals)

    print("%.5f, %.5f, %.5f, %s, %s, %d, %d" % (iavg, imin, imax, units, multiplier, start, end))
