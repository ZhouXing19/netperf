...
def find_vrules(source):
    ...
    for line in source:
        ...
        elif "Number of streams:" in line:
            netperf_count = int(line.split()[3])
        elif "Netperfs started" in line:
            fields = line.split()
            vrule = 'VRULE:%d#%s:All %d netperfs running' % (int(float(fields[3])),PURPLE,netperf_count)
            vrules.append(vrule)
...
# raw_results: content inside an *.out file
def process_result(basename, raw_results, end_time):
    first_result = True
    have_result = False
    had_results = False
    interim_result=0.0
...
# Till all .out has the first positive entry
def get_min_timestamp(prefix):
    min_timestamp = -1
    results_list = glob.glob(prefix + "*.out")
    for result_file in results_list:
        basename = result_file.replace(".out", "")
        raw_results = open(result_file, "r")
        first_timestamp, last_timestamp = process_result(basename, raw_results, end_time)
        if (first_timestamp != 0) and (last_timestamp != 0):
            min_timestamp = max(min_timestamp, first_timestamp)
...
def units_et_al_by_prefix(prefix):
    ...
    if ("pps" in prefix) or ("tps" in prefix):
        ...
        direction = "Outbound"
    elif ("search_best_num_streams" in prefix) or ("draw_plot" in prefix):
        direction = "Outbound"
...
if __name__ == '__main__':
    ...
    min_timestamp=get_min_timestamp(prefix)
    if min_timestamp == -1:
        print("There were no valid results for this prefix!")
        exit()

    vrules.append('VRULE:%d#%s:All netperfs has non zero throughput' % (int(min_timestamp),BLUE))

    start_time = max(start_time, min_timestamp)
...
#    print("Min timestamp for %s is %s start time is %s end_time is %s" % (prefix,min_timestamp,start_time,end_time))
    ...
    print("Minimum throughput: %.3f 10^%d%s" % (peak_minimum, round(math.log(multiplier, 10)), units))
    print("Average throughput: %.3f 10^%d%s" % (results_list[0][0], round(math.log(multiplier, 10)), units))
    print("Maximum throughput: %.3f 10^%d%s" % (peak_maximum, round(math.log(multiplier, 10)), units))
